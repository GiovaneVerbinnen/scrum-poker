@if ($feature)
<div @if (isManager()) wire:click="$emit('featureSelected', {{ $feature->id }})" @endif class="rounded-lg group relative p-2 my-2 cursor-pointer transition-all duration-200 border-2
...
">{{ $feature->name }}</span>
    <div class="absolute  bottom-0 right-0 p-1.5">
        <x-button wire:click.stop="remove"
            class="bg-red-400 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition duration-200">
        ...
        <x-button wire:click.stop="toggleComplete" class="bg-green-400 p-1.5 rounded-full  group-hover:opacity-100 transition duration-200
